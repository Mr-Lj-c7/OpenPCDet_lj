...
class CustomDataset(DatasetTemplate):
    ...
    def get_infos(self, num_workers=4, has_label=True, count_inside_pts=True, sample_id_list=None):
        ...
        def process_single_scene(sample_idx):
            ...
            if has_label:
                ...
                # # 满足kitti验证的需求，将图像相关属性信息加入到annotations的键值对中，即使他们默认值为0
                ...
                annotations['name'] = np.array([obj.cls_type for obj in obj_list])
                annotations['truncated'] = np.array([0.0 for obj in obj_list])
                annotations['occluded'] = np.array([0 for obj in obj_list])
                annotations['alpha'] = np.array([0.0 for obj in obj_list])
                annotations['bbox'] = np.zeros((len(obj_list), 4))  # 添加默认bbox
                annotations['dimensions'] = np.array([[obj.l, obj.h, obj.w] for obj in obj_list])  # lhw(camera) format
                annotations['location'] = np.concatenate([obj.loc.reshape(1, 3) for obj in obj_list], axis=0)
                annotations['rotation_y'] = np.array([obj.ry for obj in obj_list])
                annotations['score'] = np.array([obj.score for obj in obj_list])
    ...
    def generate_prediction_dicts(batch_dict, pred_dicts, class_names, output_path=None):
        ...
        def get_template_prediction(num_smaples):
            ...
            # }
            ret_dict = {
                'name': np.zeros(num_smaples), 'truncated': np.zeros(num_smaples),
                'occluded': np.zeros(num_smaples), 'alpha': np.zeros(num_smaples),
                'bbox': np.zeros([num_smaples, 4]), 'dimensions': np.zeros([num_smaples, 3]),
                'location': np.zeros([num_smaples, 3]), 'rotation_y': np.zeros(num_smaples),
                'score': np.zeros(num_smaples),
                'boxes_lidar': np.zeros([num_smaples, 7])
            }
            return ret_dict
        ...
        #     # modfy by lj 2025.08.05
        #     pred_dict['bbox'] = np.zeros([pred_scores.shape[0], 4])  # 添加默认的bbox
        ...
        def generate_single_sample_dict(batch_index, box_dict):
            ...
            pred_dict['name'] = np.array(class_names)[pred_labels - 1]
            pred_dict['truncated'] = np.full(pred_scores.shape[0], -1)
            pred_dict['occluded'] = np.full(pred_scores.shape[0], -10)
            pred_dict['alpha'] = np.full(pred_scores.shape[0], -10)
            pred_dict['bbox'] = np.zeros([pred_scores.shape[0], 4])
            pred_dict['dimensions'] = np.zeros([pred_scores.shape[0], 3])
            pred_dict['location'] = np.zeros([pred_scores.shape[0], 3])
            pred_dict['rotation_y'] = np.zeros(pred_scores.shape[0])
            pred_dict['score'] = pred_scores
            pred_dict['boxes_lidar'] = pred_boxes
        ...
        for index, box_dict in enumerate(pred_dicts):
            ...
            if output_path is not None:
                cur_det_file = output_path / ('%s.txt' % frame_id)
                with open(cur_det_file, 'w') as f:
                    # 使用lidar坐标下的框，而不是图像坐标下的框
                    for idx in range(len(single_pred_dict['name'])):
                        print('%s -1 -1 %.4f %.4f %.4f %.4f %.4f %.4f %.4f %.4f %.4f %.4f %.4f %.4f %.4f'
                            % (single_pred_dict['name'][idx],
                               single_pred_dict['truncated'][idx], 
                               single_pred_dict['occluded'][idx], 
                               single_pred_dict['alpha'][idx],
                                single_pred_dict['bbox'][idx][0], single_pred_dict['bbox'][idx][1], 
                                single_pred_dict['bbox'][idx][2], single_pred_dict['bbox'][idx][3],
                                single_pred_dict['dimensions'][idx][1], single_pred_dict['dimensions'][idx][2], 
                                single_pred_dict['dimensions'][idx][0], single_pred_dict['location'][idx][0],
                                single_pred_dict['location'][idx][1], single_pred_dict['location'][idx][2], 
