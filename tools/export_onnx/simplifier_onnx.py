...
def replace_with_clip(self, inputs, outputs):
    ...
    op_attrs = dict()
    # op_attrs["dense_shape"] = np.array([496,432])  # [height, width]
    op_attrs["dense_shape"] = np.array([496,480])  # 特征图尺寸[x_size,y_size],x_size = (x_max-x_min)/grid_size, y_size = (y_max-y_min)/grid_size
...
def simplify_postprocess(onnx_model):
  ...
  # 根据配置动态计算特征图尺寸
  # 从配置文件 POINT_CLOUD_RANGE: [-26.4, -39.68, -3, 50.4, 39.68, 1]
  # VOXEL_SIZE: [0.16, 0.16, 4]
  # 计算网格尺寸: x_size = (50.4 - (-26.4)) / 0.16 = 76.8 / 0.16 = 480
  #              y_size = (39.68 - (-39.68)) / 0.16 = 79.36 / 0.16 = 496
  # 经过LAYER_STRIDES: [2, 2, 2]下采样后，特征图尺寸为 [496/2, 480/2] = [248, 240]
  feature_y_size = 248  # 496 / 2
  feature_x_size = 240  # 480 / 2
  
  # 根据配置文件中的类别数量和anchor配置动态计算输出通道数
  # 4个类别（Car, Bus, Pedestrian, Motorcycle)
  num_classes = 4
  # 每个类别只有1个anchor尺寸，但有2个旋转方向，所以每个类别有2个anchors
  num_anchors_per_class = 2  # 1个尺寸 * 2个方向
  num_anchors_total = num_classes * num_anchors_per_class  # 4 * 2 = 8
  
  cls_preds_channels = num_anchors_total * num_classes  # 8 * 4 = 32 (每个anchor对每个类别进行分类)
  box_preds_channels = num_anchors_total * 7   # 8 * 7 = 56 (每个anchor预测7个参数)
  dir_cls_preds_channels = num_anchors_total * 2  # 8 * 2 = 16 (每个anchor预测2个方向)

  # 输出张量类别预测、包围框预测、方向预测，张量形状[batch_size, height, width, channels]
  # 
  '''
  cls_preds.shape = (1, 248, 240, cls_preds_channels)
  box_preds.shape = (1, 248, 240, box_preds_channels)
  dir_cls_preds.shape = (1, 248, 240, dir_cls_preds_channels)
  '''
  cls_preds = gs.Variable(name="cls_preds", dtype=np.float32, shape=(1, feature_y_size, feature_x_size, cls_preds_channels))
  box_preds = gs.Variable(name="box_preds", dtype=np.float32, shape=(1, feature_y_size, feature_x_size, box_preds_channels))
  dir_cls_preds = gs.Variable(name="dir_cls_preds", dtype=np.float32, shape=(1, feature_y_size, feature_x_size, dir_cls_preds_channels))
