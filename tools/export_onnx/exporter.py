...
# add
def calculate_feature_map_size(point_cloud_range, voxel_size):
    x_size = (point_cloud_range[3] - point_cloud_range[0]) / voxel_size[0]
    return int(x_size), int((point_cloud_range[4] - point_cloud_range[1]) / voxel_size[1])
...
def main():
    ...
    with torch.no_grad():
      ...
      MAX_VOXELS = 10000
    #   MAX_VOXELS = 40000  # 数值越大精度越高，计算速度越低
      ...
      dummy_input['batch_size'] = 1  # 1


      torch.onnx.export(
          model,       # model being run
          dummy_input,               # model input (or a tuple for multiple inputs)
          "onnx/pointpillar_raw.onnx",  # where to save the model (can be a file or file-like object)
      ...
      check = "onnx/pointpillar_raw.onnx"
      if check:
        onnx_raw_ = onnx.load("onnx/pointpillar_raw.onnx")
        for output in onnx_raw_.graph.output:
            dim_info = [dim.dim_value if dim.dim_value != 0 else "Dynamic" for dim in output.type.tensor_type.shape.dim]
            print(f"Output name: {output.name}, Shape: {dim_info}")

      onnx_raw = onnx.load("onnx/pointpillar_raw.onnx")  # load onnx model
      ...
      onnx.save(onnx_final, "onnx/pointpillar.onnx")
